...
class DMCWrapper(core.Env):
    ...
    def set_physics_state(self, state):
        new_state = self._env._physics.get_state()
        new_state[1:] = state
        with self._env._physics.reset_context():
            self._env._physics.set_state(new_state)

    def get_physics_state(self):
        return self._env._physics.get_state()

    def get_task_observation(self):
        return self._env._task.get_observation(self._env._physics)
    ...
    def get_pixel_space(self):
        shape = [3, self._height, self._width] if self._channels_first else [self._height, self._width, 3]
        return spaces.Box(low=0, high=255, shape=shape, dtype=np.uint8)

    def get_physics_shape(self):
        return self._env._physics.get_state().shape
